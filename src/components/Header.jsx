...
import { Search, Bell, Settings, ShieldAlert, LogIn, LogOut } from 'lucide-react';
...
function Header({ currentTab, setCurrentTab, user, openAuthModal, handleSignOut }) {
  ...
  return (
    <header className="sticky top-0 w-full z-50 flex justify-between items-center px-6 py-4 bg-[#0b1326]/80 backdrop-blur-xl border-b border-white/10 shadow-[0_0_30px_rgba(137,206,255,0.1)]">
      
      <div className="flex items-center gap-8">
        
        {/* 1. CHỈ GẮN SỰ KIỆN CLICK VÀO KHU VỰC LOGO */}
        <div 
        ...
        {/* 2. THANH MENU TÁCH ĐỘC LẬP HOÀN TOÀN KHỎI LOGO */}
        <nav className="hidden md:flex gap-6">
          <button 
            onClick={() => setCurrentTab('dashboard')} 
            className={`text-sm font-bold pb-1 transition-all ${currentTab === 'dashboard' ? 'text-sky-400 border-b-2 border-sky-400' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Dashboard
          </button>
          ...
          <button 
            ...
            className={`text-sm font-bold pb-1 transition-all ${currentTab === 'lessons' ? 'text-sky-400 border-b-2 border-sky-400' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Video Lessons
          </button>
          ...
          <button 
            ...
            className={`text-sm font-bold pb-1 transition-all ${currentTab === 'flashcards' ? 'text-sky-400 border-b-2 border-sky-400' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Kanji Mastery
          </button>
          ...
          <button 
            ...
            className={`text-sm font-bold pb-1 transition-all ${currentTab === 'leaderboard' ? 'text-sky-400 border-b-2 border-sky-400' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Leaderboard
          </button>
          ...
          <button 
            ...
            className={`text-sm font-bold pb-1 transition-all flex items-center gap-1 ${currentTab === 'admin' ? 'text-amber-400 border-b-2 border-amber-400' : 'text-amber-500/70 hover:text-amber-400'}`}
          >
      ...
      <div className="flex items-center gap-4">
        ...
        <button 
          ...
          className="text-slate-400 hover:bg-white/5 p-2 rounded-full transition-colors"
        >
